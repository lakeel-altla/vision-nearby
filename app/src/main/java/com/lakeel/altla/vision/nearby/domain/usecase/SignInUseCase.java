...
public final class SignInUseCase {
    ...
    public Observable<Void> execute() {
        ...
        return saveUser(userId)
                ...
                .flatMap(aVoid -> saveUserBeacon(userId, model.beaconId))
                .flatMap(aVoid -> saveBeacon(userId, model.beaconId))
                .flatMap(aVoid -> saveDeviceToken(model.beaconId));
    }

    private Observable<Void> saveUser(String userId) {
        return userProfileRepository.save(userId).subscribeOn(Schedulers.io()).toObservable();
    }
    ...
    private Observable<Void> saveBeaconId(String userId, String beaconId) {
        return preferenceRepository.saveBeaconId(userId, beaconId).subscribeOn(Schedulers.io()).toObservable();
    }

    private Observable<Void> saveUserBeacon(String userId, String beaconId) {
        return userProfileRepository.saveUserBeacon(userId, beaconId).subscribeOn(Schedulers.io()).toObservable();
    }

    private Observable<Void> saveBeacon(String userId, String beaconId) {
        ...
        return beaconRepository.save(beacon).subscribeOn(Schedulers.io()).toObservable();
    }

    private Observable<Void> saveDeviceToken(String beaconId) {
        ...
        return userDeviceTokenRepository.save(deviceToken).subscribeOn(Schedulers.io()).toObservable();
    }
